...
def shift_down(arr,node,arr_len):
    left = node * 2  #如果有右孩子,右孩子 的下标
    right = node * 2 +1 #如果有右孩子,右孩子 的下标
    print('node:%s, child:%s arr_len:%s'%(node,left,arr_len))
    while node > 0 and left  <= arr_len or right <= arr_len: #
        node_val = arr[node -1]
        left_child = arr[node*2-1]
        right_child = arr[node*2+1 -1]
        if node_val < left_child:
            arr[node-1] = left_child
            arr[node*2-1] = node_val

        elif node_val < right_child:
            arr[node-1] = right_child
            arr[node*2+1-1] =  node_val
        elif left_child > right_child:#left 不能大于右边
            arr[node*2-1] = right_child
            array[node*2+1-1] = left_child
        else:
            if node_val >= left_child and node_val >=right_child :

                print('perfect', node_val,left_child,right_child)
                break
